...
data:process/[=xpath.filter('./Project/Final_Project_ID/text()', dataset)]_[=xpath.filter('./Project/GUID/text()', dataset)]
    a process:Process ;
...
<#items as task>
data:task/[=xpath.filter('./Project/Final_Project_ID/text()', dataset)]_[=xpath.filter('./Project/GUID/text()', dataset)]_[=xpath.filter('./Task/UID/text()', task)]
    a process:Task ;
    ...
    process:hasCost data:cost/[=xpath.filter('./Task/GUID/text()', task)] <#if xpath.filter('./Task/Childs',task)?trim != '' || xpath.filter('./Task/Parent_WBS',task)?trim != ''>;<#else>.</#if>
    <#if xpath.filter('./Task/Childs',task)?trim != ''>
        ...
        <#if childs?is_sequence>
            <#list childs as child>
                '[=child]'^^<http://www.w3.org/2001/XMLSchema#string> <#if child?is_last && xpath.filter('./Task/Parent_WBS',task)?trim == ''> . <#else> ; </#if>
            </#list>
        <#else>
